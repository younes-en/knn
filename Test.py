...
def run_quiz(questions, Interpretation):
    ...
    ###############################
    val_rand = ['1', '2', '3']
    for headers in list(df):

        # On transforme les a, b, c en 1, 2, 3.
        if df[headers][0] == 'a' or df[headers][0] == 'A':
            df[headers][0] = '1'
        elif df[headers][0] == 'b' or df[headers][0] == 'B':
            df[headers][0] = '2'
        elif df[headers][0] == 'c' or df[headers][0] == 'C':
            df[headers][0] = '3'
        ...
        elif df[headers][0] not in ['1', '2', '3']:

            # Méthode 1: 0 points
            #data[headers][i] = '2'

            # Méthode 2: Nombre de points aléatoires
            df[headers][0] = val_rand[randint(0, 2)]
    # charger le model
    # scaler = StandardScaler()
    # scaler.fit(For_Test)
    # X = scaler.transform(For_Test)
    # P = model.predict(For_Test)

    model = joblib.load("KNN_brief")

    P = model.predict(For_Test)

    print('Prédiction KNN : ', P[0])
